...
function MarcaView() {
  ...
  const { category, types } = useParams()

  useEffect(() => {
    actions.getAllLiquorsMarca(category, types)
  }, [])
              ...
              <div key={item.id} className='group'>
                <Link to={`/details/${item.id}`} >
                  <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg  xl:aspect-h-8 xl:aspect-w-7">
                    <img src={item.image} alt={item.name} className="h-full w-full object-cover object-center group-hover:opacity-75" />
